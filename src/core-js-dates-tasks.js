...
function getTime(date) {
  const formatTime = (timePart) =>
    timePart > 9 ? `${timePart}` : `0${timePart}`;
...
function getDayName(date) {
  ...
  const myDate = new Date(date);
  return dayNames[myDate.getUTCDay()];
}
...
function getNextFriday(date) {
  const inputDay = date.getUTCDay();
  let daysToAdd;

  daysToAdd = (5 - inputDay + 7) % 7;
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 0; day < totalDaysInMonth; day += 1) {
    const currentDay = new Date(year, month - 1, day + 1);
